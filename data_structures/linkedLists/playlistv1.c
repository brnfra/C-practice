...
/*
        head = inicInsert(head, "Time", "Pink Floyd" , 408 );
		head = lastInsert(head,"Stairway to Heaven" , "Led Zeppelin" , 480 );
		head = midInsert(head, "Sultans of Swing" , "Dire Straits" , 346 );
...
*/
	...
	do {
		...
		if (op == 1) {
			...
			}//caractere anterior fecha a string e evita que fgets pule uma linha
...
void * apagaItem(t_list * head, int erase) {
	...
	int  segundos, achou, meio, n, opcao;
