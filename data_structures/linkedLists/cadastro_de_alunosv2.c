/* Author brnfra
        Exercício 2 Pesquisa Matrícula de Aluno
*/
...
void ClearScreen();// função limpa tela é usada no compilador gcc pois o system("cls") tem essa função 
void  menu();
void menuProcura(void);
void listaVazia(void);
void * insert(t_list * head , char * name, char * email, int RA);
int  search(t_list * head ,int match, int RA);
...
void main() {
    ...
    //declara variaveis 
    int op,apaga, nRA, achou;
    char nome[tamNome], email[tamEmail];
    char num[tamRA];//converter num em nRA
          ...
          // menu de opções
            ...
            if( op == 1 ){
                    ...
                    if (fgets(email, sizeof(email), stdin)){ //armazena o nome da bnda a ser  inserida
                      ...
                      printf("O número da matrícula (RA): ");
                      fflush(stdin);
                     if (fgets(num, sizeof(num), stdin)) {
                        if (1 == sscanf(num, "%d", &nRA)) {
                         printf("\n");
            
                        head = insert( head, nome, email, nRA );//devido a funcao de imprimir precisa retornar o end de ini
             
                        }
             }//armazena o nRA a ser  inserido
                ...
                //system("cls");
                    ...
                    if (head == NULL){
                         ...
                         menuProcura();
                             ...
                             if (fgets(num, sizeof(num), stdin)) {
                                    if (1 == sscanf(num, "%d", &nRA)) {//recebe op
                                             printf("\n");
...
void menuProcura(void){
                ...
                printf("\nQual RA do Aluno para pesquisa");
                printf("\nRA :");
    ...
    if(aux == NULL){
				...
				aux = (t_list *)malloc(sizeof(t_list));//##Aloca memoria para inicio sizeof ## é uma função que retorna o tamanho de um tipo( e nao da variavel) malloc
			        	...
			        	if(aux == NULL){
                            ...
                            aux -> RA = nRA;	    // ponteiro para RA na estrutura list o primeiro e o ultimo
                        ...
                        //aloca nova memoria para o proximo campo TRABALHANDO COM OS ENDEREÇOS
                            //faz-se uso da recursividade
                                     ...
                                     if((nRA) < (aux -> RA)){
                                            ...
                                            aux -> esq =insert(aux -> esq,  nome ,  correio,  nRA);
  

                                      }else{
                                          
                                            aux -> dir =insert(aux -> dir,  nome ,  correio,  nRA);
    ...
    achou = match;
                    ...
                    if(aux!=NULL && achou == 0){
                        ...
                        if(aux -> RA == nRA){
                            ...
                            achou = search(aux -> esq, achou , nRA);
                        }else{
                            if(aux->dir == NULL)return achou;
                            achou = search(aux -> dir, achou , nRA);
                        }
...
void * resultado(t_list * aux ,int match, int op){
     ...
     switch (operacao){
         ...
         //lista vazia
            printf("\n|########################################      RESULTADO      ########################################|\n\n");
            printf("\n %-43s%-45s%s","NOME","E-mail","Matrícula(RA)\n");
         ...
         //objeto nao encontrado
            printf("\n|########################################      RESULTADO      ########################################|\n\n");
            printf("\n %-43s%-45s%s","NOME","E-mail","Matrícula(RA)\n");
            printf("\n Matrícula(RA) não foi encontrada !!!\n");
            printf("\n\n");
            printf("\n|########################################      RESULTADO      ########################################|\n"); 
            system("pause");
         ...
         //imprime resultado encontrado
            printf("\n|########################################      RESULTADO      ########################################|\n\n");
            printf(" %-43s%-45s%s","NOME","E-mail","Matrícula(RA)\n");
            printf("\n %-43s%-45s%d\n",auxiliar->nomeAluno, auxiliar->email, auxiliar -> RA);
            printf("\n|########################################      RESULTADO      ########################################|\n");
